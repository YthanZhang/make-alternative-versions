...
void test_type(std::istream const&, char& c, int& i)
{
	static int count = 0;
	c				 = 'r';
	i				 = ++count;
...
void test_type(std::ostream const&, char& c, int& i)
{
	static int count = 0;
	c				 = 'w';
	i				 = ++count;
...
void test_type(std::iostream const&, char& c, int& i)
{
	static int count = 0;
	c				 = 'b';
	i				 = ++count;
...
void test_type(rpstream const&, char& c, int& i)
{
	static int count = 0;
	c				 = 'x';
	i				 = ++count;
...
template<typename T>
void print_result(T const& s, bool result)
{
...
template<typename T>
bool check_pass(T const& s, bool expected = true)
{
...
template<typename T>
bool check_fail(T const& s)
{
	return check_pass(s, false);
}
...
int main()
{
...

	{
		...
		const std::string cmd =
			"grep '^127' /etc/hosts /no/such/file /dev/stdin";
		...
		std::string buf;
		while (getline(ps.out(), buf)) cout << "STDOUT: " << buf << endl;
		check_fail(ps);
		ps.clear();
		while (getline(ps.err(), buf)) cout << "STDERR: " << buf << endl;
		check_fail(ps);
		ps.clear();
	}
...

	{
		...
		print_result(ofail, WIFEXITED(status) && WEXITSTATUS(status) == sh_cmd_not_found);
	}
...

	{
		...
		while (in && avail >= 0)
		{
			if (avail > 0)
			{
				...
				cout << "STDOUT: " << avail
					 << " characters: " << std::string(buf.begin(), buf.end()) << endl;
			}
			else
				++tries;
